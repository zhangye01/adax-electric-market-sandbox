...
| `src/components/retail/RetailExecutionResultPanel.tsx` | 152 | `src/components` |
...
| --- | --- |
| `src/styles.css` | 118 |
| `src/components/retail/RetailExecutionWorkspace.tsx` | 15 |
...
33. `scripts/check-boundaries.mjs` now treats those calculation helper modules as reviewed domain-rule targets, so components and pages cannot bypass the calculation facade without an explicit architecture exception. `tests/scripts/check-boundaries.test.mjs` now guards representative false-negative cases for validation imports, browser APIs in domain modules, and real province runtime data.
...
37. `scripts/check-domain-contracts.mjs` now guards the reviewed `retailTypes.ts` and `src/types.ts` export groups and order so accidental contract expansion, renaming, or category drift cannot pass `npm run quality` unnoticed. Contract governance is documented in `docs/ADAX_RETAIL_CONTRACT_GOVERNANCE.md`, and `tests/scripts/check-domain-contracts.test.mjs` proves the negative paths.
38. `scripts/check-source-shape.mjs` now has script-level negative fixtures for unbudgeted watch-threshold files and budgeted-file growth, so the source-shape gate itself cannot silently stop enforcing line-pressure rules.
39. `src/components/Layout.tsx` has been reduced from a budgeted app-shell pressure file to a shell coordinator. Sidebar navigation, topbar context, and the market-clearing brand mark now live in `src/components/layout/**`.
