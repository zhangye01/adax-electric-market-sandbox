...
| Manual Pages publishing missing a step | Reduced | Use `npm run publish:pages:dry` before `npm run publish:pages -- --yes`; publishing logic is centralized in `scripts/publish-pages.mjs`, `tests/scripts/publish-pages.test.mjs` rejects real publishing when quality is skipped, and `npm run check:engineering-guardrails` verifies the dry-run command, real publish command, quality gate, and `gh-pages` push target |
