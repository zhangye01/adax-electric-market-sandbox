...
| Engineering gate docs, quality/publishing pipeline, or closed participant scope drifting | Reduced | `npm run check:engineering-guardrails` fails if required governance docs are missing, key entry documents lose required references, `package.json` loses required quality or publishing commands, release-process safeguards drift, script-level tests are removed, Phase 5 candidate gates stop saying implementation is closed, or closed participant runtime files appear in active source; `tests/scripts/check-engineering-guardrails.test.mjs` proves missing-doc, missing-reference, accidental-opening, missing-quality-command, publishing-drift, missing-test-target, closed-runtime-file, and legacy-reference fixtures |
...
| Manual Pages publishing missing a step | Reduced | Use `npm run publish:pages:dry` before `npm run publish:pages -- --yes`; publishing logic is centralized in `scripts/publish-pages.mjs`, and `npm run check:engineering-guardrails` verifies the dry-run command, real publish command, quality gate, and `gh-pages` push target |
