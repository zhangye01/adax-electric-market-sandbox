...
Use `docs/ADAX_HARDENING_DECISION_PACKET.md` as the user-facing decision packet before asking whether to keep hardening, lift Engineering Hardening Hold for one participant, or enter Project Rescue.
...
Use `docs/ADAX_FEATURE_RESTART_REHEARSAL.md` as the latest no-code rehearsal of the feature-resumption gates before treating the runbook as implementation-ready.
...
`npm run quality` runs the engineering guardrail check, boundary check, central contract check, and source-shape budget check before typecheck, tests, and build. The engineering guardrail check also verifies that the package quality and publishing pipelines still contain the required commands, every `tests/**/*.test.mjs` file is wired into `npm run test`, source-repository build/cache/test artifacts remain untracked, GitHub workflow files remain absent under the current local-script Pages strategy, candidate startup cards remain pending user confirmation, release-process safeguards remain documented, and closed Phase 5 participant runtime files have not appeared in active source.
...
| Engineering gate docs, quality/publishing pipeline, source-repository artifact boundary, hardening-exit evidence, or closed participant scope drifting | Reduced | `npm run check:engineering-guardrails` fails if required governance docs are missing, key entry documents lose required references, `package.json` loses required quality or publishing commands, release-process safeguards drift, test files are removed or left unwired, generated artifacts are tracked in the source repository, GitHub workflow files appear before the Pages strategy is intentionally changed, the hardening exit audit loses its verification snapshot or freshness rule, Phase 5 candidate gates stop saying implementation is closed, or closed participant runtime files appear in active source; `tests/scripts/check-engineering-guardrails.test.mjs` proves missing-doc, missing-reference, stale-exit-audit, accidental-opening, missing-quality-command, publishing-drift, missing-test-target, unwired-test-file, source-artifact, workflow-file, closed-runtime-file, and legacy-reference fixtures |
...
| Test fixtures drifting through copy-paste or tests being added without execution | Reduced | Shared retail fixtures live in `tests/support/retail-fixtures.mjs`; shared fake browser storage setup lives in `tests/support/browser-fixtures.mjs`; ADAX common domain tests live in `tests/domain/adax-domain.test.mjs`; route tests live in `tests/routes/adax-routes.test.mjs`; template IO tests live in `tests/services/retail-execution-templates.test.mjs`; localStorage tests live in `tests/utils/adax-storage.test.mjs`; app navigation action tests live in `tests/app/adax-navigation-actions.test.mjs`; `npm run check:engineering-guardrails` rejects any `tests/**/*.test.mjs` file that is not wired into `npm run test` |
| New participant implementation starting before scope confirmation | High | Require a confirmed participant startup card before code; current candidate cards are `docs/ADAX_RENEWABLE_STARTUP_CARD.md`, `docs/ADAX_INDEPENDENT_STORAGE_STARTUP_CARD.md`, and `docs/ADAX_THERMAL_STARTUP_CARD.md`, all pending confirmation; `docs/ADAX_FEATURE_RESTART_REHEARSAL.md` proves the current gates still block a hypothetical renewable slice; `docs/ADAX_FEATURE_IMPLEMENTATION_RUNBOOK.md` defines the required domain-first, test-backed implementation order after confirmation; `npm run check:engineering-guardrails` fails if those pending statements, rehearsal blockers, or runbook stop rules are removed |
...
| Closed participant records or materials leaking into active localStorage | Reduced | `src/utils/adaxStorage.ts` and `src/services/adaxUserMaterials.ts` keep active records and user materials limited to `retailer` while Phase 5 is closed; `tests/utils/adax-storage.test.mjs` covers closed participant record read/save and material read/save/upsert behavior |
...
| Source `main` push failure being confused with rollback or Pages publishing | Medium | Follow the source remote sync recovery rules in `docs/ADAX_RELEASE_PROCESS.md`: compare `origin/main...HEAD`, check `git ls-remote origin HEAD`, retry normal push after network recovery, and do not use `git reset --hard` or force-push `main` for ordinary HTTPS write failures |
| Build or cache artifacts polluting source history | Reduced | Keep `dist/`, `coverage/`, `.vite/`, and `.test-build/` untracked in `main`; `npm run check:engineering-guardrails` rejects tracked generated artifacts before quality continues |
| Manual Pages publishing missing a step | Reduced | Use `npm run publish:pages:dry` before `npm run publish:pages -- --yes`; publishing logic is centralized in `scripts/publish-pages.mjs`, dry-run reports whether the static release directory matches current `dist/`, `tests/scripts/publish-pages.test.mjs` rejects real publishing when quality is skipped and proves dry-run release-delta diagnostics, and `npm run check:engineering-guardrails` verifies the dry-run command, real publish command, quality gate, and `gh-pages` push target |
| GitHub workflow files blocked by token scope | Reduced | Current GitHub CLI token lacks `workflow`; `npm run check:engineering-guardrails` rejects `.github/workflows/**` under the current local-script Pages strategy unless authorization and Pages strategy are intentionally changed |
...
| Training action orchestration becoming a mixed route/action god module | Reduced | Navigation actions now live in `src/app/createAdaxNavigationActions.ts` with direct tests in `tests/app/adax-navigation-actions.test.mjs`; keep record/material/template coordination in `createAdaxTrainingActions.ts`; `tests/app/adax-training-actions.test.mjs` covers mode reset, execution result/save guards, and node-bound review material saves without browser history |
