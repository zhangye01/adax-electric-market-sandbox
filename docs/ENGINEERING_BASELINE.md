...
| Test fixtures drifting through copy-paste or tests being added without execution | Reduced | Shared retail fixtures live in `tests/support/retail-fixtures.mjs`; shared fake browser storage setup lives in `tests/support/browser-fixtures.mjs`; route tests live in `tests/routes/adax-routes.test.mjs`; template IO tests live in `tests/services/retail-execution-templates.test.mjs`; localStorage tests live in `tests/utils/adax-storage.test.mjs`; `npm run check:engineering-guardrails` rejects any `tests/**/*.test.mjs` file that is not wired into `npm run test` |
