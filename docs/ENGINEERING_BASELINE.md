...
| Engineering gate docs, quality/publishing pipeline, source-repository artifact boundary, or closed participant scope drifting | Reduced | `npm run check:engineering-guardrails` fails if required governance docs are missing, key entry documents lose required references, `package.json` loses required quality or publishing commands, release-process safeguards drift, script-level tests are removed, generated artifacts are tracked in the source repository, Phase 5 candidate gates stop saying implementation is closed, or closed participant runtime files appear in active source; `tests/scripts/check-engineering-guardrails.test.mjs` proves missing-doc, missing-reference, accidental-opening, missing-quality-command, publishing-drift, missing-test-target, source-artifact, closed-runtime-file, and legacy-reference fixtures |
