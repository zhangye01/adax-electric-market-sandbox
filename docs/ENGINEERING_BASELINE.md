...
| Engineering gate docs, quality pipeline, or closed participant scope drifting | Reduced | `npm run check:engineering-guardrails` fails if required governance docs are missing, key entry documents lose required references, `package.json` loses required quality commands or script-level tests, Phase 5 candidate gates stop saying implementation is closed, or closed participant runtime files appear in active source; `tests/scripts/check-engineering-guardrails.test.mjs` proves missing-doc, missing-reference, accidental-opening, missing-quality-command, missing-test-target, closed-runtime-file, and legacy-reference fixtures |
