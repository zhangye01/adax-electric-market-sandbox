...
6. Run `npm run quality` for code changes. It includes boundary checks, central domain contract checks, source-shape budgets, typecheck, tests, and build.
...
- 独立储能 startup card created: `docs/ADAX_INDEPENDENT_STORAGE_STARTUP_CARD.md`.
- 火电 startup card created: `docs/ADAX_THERMAL_STARTUP_CARD.md`.
- Phase 5 candidate readiness audit created: `docs/ADAX_PHASE_5_CANDIDATE_READINESS_AUDIT.md`.
- Phase 5 entry gate rehearsal created: `docs/ADAX_PHASE_5_ENTRY_GATE_REHEARSAL.md`.
- Phase 5 scope-control matrix created: `docs/ADAX_PHASE_5_SCOPE_CONTROL_MATRIX.md`.
...
- Feature resumption decision checklist created: `docs/ADAX_FEATURE_RESUMPTION_DECISION_CHECKLIST.md`.
- Feature implementation runbook created: `docs/ADAX_FEATURE_IMPLEMENTATION_RUNBOOK.md`.
- Feature restart rehearsal created: `docs/ADAX_FEATURE_RESTART_REHEARSAL.md`.
...
Current user-decision packet: `docs/ADAX_HARDENING_DECISION_PACKET.md`.

Current feature-resumption gate: `docs/ADAX_FEATURE_RESUMPTION_DECISION_CHECKLIST.md`.

Current feature implementation runbook: `docs/ADAX_FEATURE_IMPLEMENTATION_RUNBOOK.md`.

Current feature restart rehearsal: `docs/ADAX_FEATURE_RESTART_REHEARSAL.md`.

Next recommended task: present `docs/ADAX_HARDENING_DECISION_PACKET.md` and wait for an explicit user decision before lifting Engineering Hardening Hold. Keep the engineering-readiness audit, exit audit, hardening decision packet, feature-resumption checklist, feature implementation runbook, and feature restart rehearsal current only when source, guardrail, quality, release, or active-scope evidence changes. Use `docs/ADAX_CHANGE_GATE_CHECKLIST.md`, `docs/ADAX_PHASE_5_CANDIDATE_READINESS_AUDIT.md`, `docs/ADAX_PHASE_5_ENTRY_GATE_REHEARSAL.md`, and `docs/ADAX_PHASE_5_SCOPE_CONTROL_MATRIX.md` before any new feature work. If a participant startup card is confirmed later, use `docs/ADAX_FEATURE_RESTART_REHEARSAL.md` as the last no-code gate sanity check and `docs/ADAX_FEATURE_IMPLEMENTATION_RUNBOOK.md` to keep the first implementation slice domain-first, test-backed, participant-isolated, and reversible. Keep engineering-guardrail, source-shape, domain-contract, boundary-checker, app-action, route-sync decision, session-derivation, and shared-fixture guardrails in the normal quality gate. For code structure work, keep `src/domain/retailTypes.ts` stable under `docs/ADAX_RETAIL_CONTRACT_GOVERNANCE.md` unless a confirmed participant startup card requires shared contracts. If the user chooses renewable, use `docs/ADAX_PHASE_5_RENEWABLE_ENTRY_DRY_RUN.md` before implementation. If the user chooses independent storage, use `docs/ADAX_PHASE_5_STORAGE_ENTRY_DRY_RUN.md` before implementation. If the user chooses thermal, use `docs/ADAX_PHASE_5_THERMAL_ENTRY_DRY_RUN.md` before implementation. Next code-focused hardening should extract app action test harnesses only if more stateful action scenarios are added. Do not enter Phase 5, or review or implement `docs/ADAX_RENEWABLE_STARTUP_CARD.md`, `docs/ADAX_INDEPENDENT_STORAGE_STARTUP_CARD.md`, or `docs/ADAX_THERMAL_STARTUP_CARD.md`, until the user explicitly resumes participant expansion and confirms the target participant startup card.
