...
Next recommended task: use `docs/ADAX_SOURCE_SHAPE_AUDIT.md` to review `src/styles/002-app-sidebar-panel.css` when sidebar styles are next touched. Split collapse control, brand/market mark, collapsed-state rules, and mode-card rules only if the file still mixes those responsibilities. Do not review or implement `docs/ADAX_RENEWABLE_STARTUP_CARD.md` until the user explicitly resumes participant expansion.
